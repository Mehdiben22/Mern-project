   ...
   useEffect(() => {
    const fetchPosts = async() => {
        try {
          ...
          if(res.ok) {
            ...
            // la condition qu'on vient de faire est si le nombre de posts est < 9 
    ...
    //have the useeffect each time when user changed 
     ...
     const handleShowMore = async () => {
        ...
        try {
            //&startIndex=${startIndex} doit etre coller a ${currentUser._id} pour que ca marche
              ...
              if(res.ok) {
                //Keep the previous posts and add the new results (data.posts) to the previous one
      ...
      const handleDeletePost = async () => {
        ...
        try {
           ...
           if(!res.ok) {
            ...
            //keep the previous post and filter the one who has been deleted
            setUserPosts((prev) => 
            // we get each posts and then if the post ID is not equal to the one that we are deleting 
            ...
            {userPosts.map((post) => (
             ...
             <Table.Body className='divide-y' key={post._id}>
              <Table.Row className='bg-white dark:border-gray-700 dark:bg-gray-800'>
                ...
                <Table.Cell>
                    {/* les images de la table et quand on clique sur l'image ca nous mène dans le 
                    post en relation avec l'image */}
