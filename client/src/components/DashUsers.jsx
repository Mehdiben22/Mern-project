   ...
   useEffect(() => {
    const fetchUsers = async() => {
        try {
          ...
          if(res.ok) {
            setUsers(data.users);
            // la condition qu'on vient de faire est si le nombre de users est < 9 
    ...
    //have the useeffect each time when user changed 
     ...
     const handleShowMore = async () => {
        ...
        try {
            //startIndex is for starting fetching  after 9 element 
              ...
              if(res.ok) {
                //Keep the previous users and add the new results (data.users) to the previous one
     ...
     const handleDeleteUser = async () => {
        ...
        try {
            ...
            const data = await res.json();
            if(res.ok)   {
                //Filter out the user that we have deleted
