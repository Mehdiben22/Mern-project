...
export default function PostPage() {
  ...
  return <main className="p-3 flex flex-col max-w-6xl mx-auto min-h-screen">
    ...
    {/* If there is a post so show the category same thing for all */}
    <Button color="gray" pill size='xs' >{post && post.category}</Button>
       ...
       <div className="flex justify-between p-3 border-b border-slate-500 mx-auto
        w-full max-w-2xl text-sm">
        <span>{post && new Date(post.createdAt).toLocaleDateString()}</span>
        <span className="italic">{post && (post.content.length / 1000).toFixed(0)} mins read</span>
       </div> 
       {/* post-content its just customized based on post.content  */}
      ...
      {/* comment section need the id of the post ! */}
      <CommentSection postId={post._id}/>
      <div className="flex flex-col justify-center items-center mb-5">
        <h1 className="text-xl mt-5">Recent articles</h1>
        {/* flex wrap automaticly change the numbers of collons at the beginning it's horizontal */}
